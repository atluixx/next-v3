...
function normalizeInput(
	text: string,
): AkinatorAnswer | "back" | "quit" | undefined {
...
export default {
	...
	async execute(m, c, args) {
		...
		if (!session) {
			await c.reply(
				m.chat.id,
				`⚠️ Nenhuma sessão ativa.\nEnvie \`${prefix}akinator\` para começar.`,
				m.id,
			);
			return;
		}
		...
		if (api.isWin) {
			await c.reply(
				m.chat.id,
				`✅ O jogo já terminou!\nEnvie \`${prefix}akinator\` para começar de novo.`,
				m.id,
			);
			return;
		}
		...
		if (normalized === "quit") {
			...
			await c.reply(
				m.chat.id,
				`❌ *Jogo encerrado.*\nEnvie \`${prefix}akinator\` para começar outro.`,
				m.id,
			);
			return;
		}
		...
		const enumKey = Object.keys(AkinatorAnswer).find(
			(k) => AkinatorAnswer[k as keyof typeof AkinatorAnswer] === normalized,
		);
		if (enumKey) history.push(enumKey);

		if (api.isWin) {
			const caption =
				`🎯 Eu acho que é: *${api.sugestion_name}*\n` +
