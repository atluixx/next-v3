...
import { generateCanvas } from "../../functions/render_hangman.ts";
...
interface HangmanGame {
	...
	player: { id: ContactId; name: string };
	active: boolean;
}

const games: Record<string, HangmanGame> = {};
const client = new InferenceClient(process.env.HUGGINGFACE_API_KEY);

async function fetchWordHintDescription(): Promise<{
	word: string;
	hint: string;
	description: string;
}> {
...
async function updateStats(
	...
	const current: HangmanStats = (user.stats
		.hangman as unknown as HangmanStats) || {
		games: 0,
		wins: 0,
		draws: 0,
		losses: 0,
	...
	await c.db.stats.update({
		...
		data: {
			hangman: updated as unknown as Prisma.InputJsonValue,
		},
	});
}
...
export default {
	...
	async execute(m: Message, c: ExpandedClient, args: string[]) {
		...
		if (game.player.id !== playerId) {
			await c.sendReplyWithMentions(
				chatId,
				`⚠️ Apenas ${game.player.name} pode jogar esta rodada.`,
				m.id,
			);
			return;
		}
		...
		if (!args[0]) {
			await c.sendReplyWithMentions(
				chatId,
				`❌ Envie uma letra ou a palavra inteira.`,
				m.id,
			);
			return;
		}
		...
		if (
			guess.length > 1 &&
			(!/^[a-z]+$/.test(guess) || guess.length !== normalizedWord.length)
		) {
			await c.sendReplyWithMentions(
				chatId,
		...
		if (guess.length === 1) {
			if (
				!game.guessedLetters.includes(guess) &&
				!game.wrongGuesses.includes(guess)
			) {
		...
		const normalizedGuessedLetters = game.guessedLetters.map((l) =>
			l.toLowerCase(),
		);
		const allGuessed = Array.from(new Set(normalizedWord.split(""))).every(
			(l) => normalizedGuessedLetters.includes(l),
		);
		const gameOver =
			allGuessed || game.wrongGuesses.length >= game.maxWrongGuesses;
