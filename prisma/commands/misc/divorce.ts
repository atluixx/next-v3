...
interface DivorceConfirmation {
	marriageId: number;
	requesterId: string;
}

const pendingDivorces: DivorceConfirmation[] = [];
	...
	async execute(m: Message, c: ExpandedClient) {
		...
		if (!marriage) {
			await c.sendReplyWithMentions(
				chatId,
				`❌ Você não está casado(a) atualmente.`,
				m.id,
			);
			return;
		}
		...
		const partner =
			marriage.partner1.id === senderId ? marriage.partner2 : marriage.partner1;

		// Handle confirmation
		if (arg === "confirmar") {
			const pendingIndex = pendingDivorces.findIndex(
				(p) =>
					p.marriageId === marriage.marriage_id && p.requesterId === senderId,
			);
