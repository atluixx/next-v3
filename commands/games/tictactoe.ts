import type { ContactId, Message } from "@open-wa/wa-automate";
import type { Prisma } from "@prisma/client";
import type { Command, ExpandedClient } from "@/next";
import { renderFlatMinimalTicTacToe } from "../../functions/render_ttt.ts";
...
interface Game {
	board: string[][];
	players: Record<ContactId, Player>;
	order: ContactId[];
	current: "X" | "O";
	active: boolean;
}
...
const pendingChallenges: Record<ContactId, Challenge> = {};
...
async function updateStats(
	...
	for (const [id, player] of Object.entries(players)) {
		...
		const current: TTTStats = (user?.stats?.ttt as unknown as TTTStats) || {
			games: 0,
			wins: 0,
			draws: 0,
			losses: 0,
		...
		const updated: TTTStats = {
			...
			streak,
			winrate:
				((current.wins + (isWinner ? 1 : 0)) / (current.games + 1)) * 100,
		};
		...
		await c.db.user.update({
			...
			data: {
				stats: {
					update: {
						ttt: updated as unknown as Prisma.InputJsonValue, // ✅ cast aqui
					},
				},
			},
		});
...
export default {
	...
	async execute(m: Message, c: ExpandedClient, args: string[]) {
		...
		const playerId: ContactId = m.sender.id;
		...
		if (pendingChallenges[playerId]) {
			...
			if (arg === "recusar") {
				...
				await c.sendReplyWithMentions(
					m.chat.id,
					`❌ Desafio recusado por ${playerName}.`,
					m.id,
				);
				return;
			}
			await c.sendReplyWithMentions(
				m.chat.id,
				"⚠️ Digite `aceitar` ou `recusar` para responder ao desafio.",
				m.id,
			);
			return;
		}
		...
		if (!argRaw) {
			...
			if (Object.keys(game.players).length >= 2) {
				await c.sendReplyWithMentions(
					m.chat.id,
					"⚠️ A partida já tem 2 jogadores.",
					m.id,
				);
				return;
			}
		...
		if (!Number.isNaN(Number(argRaw))) {
			...
			if (player.symbol !== game.current) {
				await c.sendReplyWithMentions(
					m.chat.id,
					`⏳ Não é sua vez. Agora é a vez do jogador ${game.current}.`,
					m.id,
				);
				return;
			}
			...
			if (cell < 1 || cell > 9) {
				await c.sendReplyWithMentions(
					m.chat.id,
					"❌ Movimento inválido. Use um número entre 1 e 9.",
					m.id,
				);
				return;
			}
			...
			if (result) {
				if (result === "draw") {
					...
					const winnerEntry = Object.entries(game.players).find(
						([, p]) => p.symbol === result,
					);
					if (winnerEntry) {
				...
				const nextPlayerEntry = Object.entries(game.players).find(
					([, p]) => p.symbol === nextSymbol,
				);
				const nextPlayer = nextPlayerEntry?.[1];
				if (nextPlayer)
					caption = `♟ Vez de ${nextPlayer.name} (${nextPlayer.symbol})`;
			}
