...
const pendingProposals: MarriageProposal[] = [];
	...
	async execute(m: Message, c: ExpandedClient, args: string[]) {
		...
		if (!args[0]) {
			await c.sendReplyWithMentions(
				chatId,
				`❌ Você precisa informar um argumento ou usuário.`,
				m.id,
			);
			return;
		}
		...
		if (command === "aceitar" || command === "recusar") {
			...
			const proposalIndex = pendingProposals.findIndex(
				(p) => p.targetId === senderId,
			);
			if (proposalIndex === -1) {
				await c.sendReplyWithMentions(
					chatId,
					`❌ Você não tem nenhuma proposta pendente.`,
					m.id,
				);
				return;
			}
			...
			if (command === "aceitar") {
				...
				await c.sendReplyWithMentions(
					chatId,
					`💍 ${targetUser.name ?? "Alguém"} aceitou a proposta de casamento de ${senderUser.name ?? "Alguém"}!`,
					m.id,
				);
			} else {
				await c.sendReplyWithMentions(
					chatId,
					`❌ ${targetUser.name ?? "Alguém"} recusou a proposta de casamento de ${senderUser.name ?? "Alguém"}.`,
					m.id,
				);
			}
		...
		if (mentionedId === senderId) {
			await c.sendReplyWithMentions(
				chatId,
				`❌ Você não pode se casar consigo mesmo(a).`,
				m.id,
			);
			return;
		}
		...
		if (existingMarriage) {
			await c.sendReplyWithMentions(
				chatId,
				`❌ Um dos usuários já está casado.`,
				m.id,
			);
			return;
		}
		...
		await c.sendReplyWithMentions(
			chatId,
			`💌 Proposta enviada para ${targetUser.name ?? "Alguém"}! Ele deve responder com "${c.prefix}proposta aceitar" ou "${c.prefix}proposta recusar".`,
			m.id,
		);
	},
} as Command;
