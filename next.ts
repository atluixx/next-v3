...
export interface Command {
	...
	execute: (m: Message, c: ExpandedClient, args?: string[]) => Promise<unknown>;
}
...
export interface ExpandedClient extends Client {
	...
	LC: () => Promise<void>;
	prefix: string;
	id: Promise<ContactId>;
	commands: Map<string, unknown>;
	admins: string[];
}

async function setProperties(c: ExpandedClient) {
	c.AC = async (g: GroupChatId, s: ContactId) => {
		try {
	...
	c.prefix = ".";
	c.db = prisma;
	c.commands = new Map<string, Command>();
...
let client: ExpandedClient;

async function setup(c: Client): Promise<void> {
	client = c as ExpandedClient;
