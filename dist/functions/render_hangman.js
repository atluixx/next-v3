...
registerFont("./fonts/Shippori.ttf", {
    family: "Shippori Antique Embolden",
});
export async function generateCanvas({ word, guessedLetters, wrongGuesses, fullGuess = null, maxWrongGuesses, hint, texts = {
    ...
    const background = await loadImage("./icons/background.png");
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
    ctx.globalAlpha = 1;
    let underscoreBaseY = 0;
    // --- DRAW UNDERSCORES ---
    const drawUnderscores = () => {
        ...
        lines.forEach((line, lineIndex) => {
            ...
            for (const originalLetter of line) {
                if (originalLetter === " ") {
                    startX += 65;
                    continue;
                }
                ...
                ctx.lineWidth = 4;
                ctx.stroke();
                if (guessedLetters.map((l) => normalize(l)).includes(normalizedLetter)) {
    ...
    let isWin = false;
    if (fullGuess) {
        if (normalize(fullGuess) === normalizedWord) {
    ...
    const attemptsColor = remaining <= 2 ? "#FF0000" : remaining <= 4 ? "#FFA500" : "#00CC00";
    const attemptsLabel = `${texts.attemptsLeft} `;
    const attemptsValue = `${remaining}`;
    const attemptsTextWidth = ctx.measureText(attemptsLabel).width + ctx.measureText(attemptsValue).width;
    ...
    ctx.fillStyle = "#000";
    ctx.textAlign = "center";
    ctx.fillText(`${texts.totalLetters} ${word.length}`, centerX, ay + 35);
    // --- HINT ---
    if (hint && hint.trim() !== "") {
    ...
    const parts = [
        ...
        () => {
            ...
            ctx.lineTo(centerX - legLength, gallowsTop + headRadius * 2 + bodyLength + 30);
            ctx.stroke();
        },
        () => {
            ctx.beginPath();
    ...
    if (!isWin) {
        ...
        isWin = normalizedWordLetters.every((letter) => normalizedGuesses.includes(letter));
    }
