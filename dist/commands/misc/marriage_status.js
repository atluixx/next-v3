...
export default {
  ...
  async execute(m, c) {
    ...
    if (!marriage || !marriage.since) {
      await c.sendReplyWithMentions(chatId, `❌ Você não está casado(a) no momento.`, m.id);
      return;
    }
    const partner = marriage.partner1.id === userId ? marriage.partner2 : marriage.partner1;
    const marriedSince = DateTime.fromJSDate(marriage.since, {
      zone: "America/Sao_Paulo",
    }).toLocaleString(DateTime.DATETIME_MED);
    // Calculate duration
    const now = DateTime.now().setZone("America/Sao_Paulo");
    const since = DateTime.fromJSDate(marriage.since).setZone("America/Sao_Paulo");
    const duration = now.diff(since, ["years", "months", "days"]).toObject();
    const durationText = `${Math.floor(duration.years ?? 0)} anos, ${Math.floor(duration.months ?? 0)} meses e ${Math.floor(duration.days ?? 0)} dias`;
    let divorceStatus = "";
    if (marriage.divorceRequested) {
      if (marriage.divorceRequesterID === userId) {
        divorceStatus = "⚠️ Você solicitou o divórcio. Aguarde a resposta do parceiro(a).";
      } else {
