...
export default {
    ...
    async execute(m, c) {
        ...
        // Handle confirmation
        if (arg === "confirmar") {
            const pendingIndex = pendingDivorces.findIndex((p) => p.marriageId === marriage.marriage_id && p.requesterId === senderId);
            if (pendingIndex === -1) {
                await c.sendReplyWithMentions(chatId, `❌ Você não possui um pedido de divórcio pendente para confirmar.`, m.id);
                return;
            }
            // Finalize divorce
            ...
            await c.sendReplyWithMentions(chatId, `💔 Divórcio confirmado entre ${marriage.partner1.name ?? marriage.partner1.id} e ${marriage.partner2.name ?? marriage.partner2.id}.`, m.id);
            return;
        }
        // Otherwise, initiate divorce request
        if (marriage.divorceRequested) {
            await c.sendReplyWithMentions(chatId, `⚠️ Um pedido de divórcio já está pendente. Aguarde a confirmação do outro parceiro(a).`, m.id);
            return;
        }
        ...
        await c.sendReplyWithMentions(chatId, `⚠️ Tem certeza que deseja divorciar-se de ${partner.name ?? partner.id}? Use ".divorciar confirmar" para finalizar.`, m.id);
    },
};
