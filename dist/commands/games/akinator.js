...
function normalizeInput(text) {
    ...
    if (["sim", "yes", "s"].includes(t))
        return AkinatorAnswer.Yes;
    if (["n", "não", "nao", "no"].includes(t))
        return AkinatorAnswer.No;
    if (["não sei", "naosei", "idk", "don't know"].includes(t))
        return AkinatorAnswer["Don't know"];
    if (["provavelmente", "probably"].includes(t))
        return AkinatorAnswer.Probably;
    if (["provavelmente não", "provavelmente nao", "probably not"].includes(t))
        return AkinatorAnswer["Probably not"];
    if (["voltar", "back"].includes(t))
        return "back";
    if (["sair", "quit", "parar", "exit"].includes(t))
        return "quit";
    return undefined;
}
...
export default {
    name: "akinator",
    description: "Jogue com o Akinator via WhatsApp. Use 'akinator' para iniciar e 'akinator sim|não|...' para responder.",
    async execute(m, c, args) {
        const prefix = c.prefix || "";
        const chatId = m.chat.id;
        const userArg = args?.[0]?.trim();
        const session = sessions.get(chatId);
        const wantsStart = !session ||
            (userArg &&
                ["reset", "start", "novo", "reiniciar"].includes(userArg.toLowerCase()));
        if (wantsStart) {
            ...
            await c.reply(m.chat.id, `🧠 *Akinator iniciado!*\n\n` +
                ...
                `🔙 \`${prefix}akinator voltar\` para desfazer\n❌ \`${prefix}akinator sair\` para encerrar`, m.id);
            return;
        }
        if (!session) {
        ...
        if (!userArg) {
            await c.reply(m.chat.id, `⚡ Você precisa responder!\n\n` +
                `❓ *Pergunta:* ${api.question}\n` +
                `📊 *Progresso:* ${api.progress.toFixed(2)}%`, m.id);
            return;
        }
        const normalized = normalizeInput(userArg);
        if (normalized === undefined) {
            await c.reply(m.chat.id, `🚫 Resposta não reconhecida.\n\nUse:\n` +
                `• \`${prefix}akinator sim\`\n` +
                `• \`${prefix}akinator não\`\n` +
                `• \`${prefix}akinator não sei\`\n` +
                `• \`${prefix}akinator provavelmente\`\n` +
                `• \`${prefix}akinator provavelmente não\`\n\n` +
                `🔙 \`${prefix}akinator voltar\`\n❌ \`${prefix}akinator sair\``, m.id);
            return;
        }
        if (normalized === "quit") {
        ...
        if (normalized === "back") {
            try {
                ...
                history.pop();
                await c.reply(m.chat.id, `⏪ *Você voltou um passo!*\n\n` +
                    `❓ *Pergunta:* ${api.question}\n` +
                    `📊 *Progresso:* ${api.progress.toFixed(2)}%`, m.id);
            }
            catch {
        ...
        await c.reply(m.chat.id, `❓ *Pergunta:* ${api.question}\n` +
            `📊 *Progresso:* ${api.progress.toFixed(2)}%\n\n` +
            `👉 Responda com:\n` +
            `• \`${prefix}akinator sim\`\n` +
            `• \`${prefix}akinator não\`\n` +
            ...
            `• \`${prefix}akinator provavelmente\`\n` +
            `• \`${prefix}akinator provavelmente não\`\n\n` +
            `🔙 \`${prefix}akinator voltar\`  |  ❌ \`${prefix}akinator sair\``, m.id);
    },
};
